...
- [ ] The validated policy is one of two mutually exclusive forms — a fixed token threshold (a positive integer,
...
- [ ] An unusable value fails the plugin at load and is never clamped, dropped or replaced: `tokens` must be a
      positive integer, `share` a finite ratio strictly inside `(0, 1)`.
- [ ] The fixed form reports one raw result whose estimated price strictly exceeds the threshold, and requires no
...
- [ ] The notice's body names the tool, the estimated price and the configured threshold, appends the price as a
...
- [ ] Nothing about delivery or state changes: the report is attached to the result that produced it, a
...
- [ ] The capacity statement still does not announce the rule, and the reading tool, the pressure tiers, the
      reminder epoch and the route-coherence gate behave exactly as before.
- [ ] The README's summary line and its documented default configuration block, and the bundle patch row's
      comment, state the form actually in force; the declared row still boots unchanged.
- [ ] Tests, at the existing seams and no new ones: the pure suite covers both forms' eligibility and boundary,
